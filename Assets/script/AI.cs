   ...
   // float sideVelocity;
    ...
    void Update()
    {
        ...
        if (grounded &&  startRandomNess)
        {
            ...
            if (distance < 20 && continueChasing && !destination)
            {
                chase = true;
            }
            else
    ...
    {
        if(other.gameObject.CompareTag("Player") && !destination)
        {
            Instantiate(GeneralVariable.instance.bikeExplosion, other.transform.position, Quaternion.identity);        //particle effect
            other.gameObject.SetActive(false);    //player disable

            PauseMenu.instance.WhenPlayerDies();
